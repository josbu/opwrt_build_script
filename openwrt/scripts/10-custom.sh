...
if curl -s "$mirror/openwrt/24-config-common" | grep -q "^CONFIG_PACKAGE_luci-app-mihomo=y"; then
    ...
    echo "src/gz mihomo https://raw.ihtw.moe/$github/morytyann/OpenWrt-mihomo/raw/gh-pages/openwrt-24.10/$arch/mihomo" >> files/etc/opkg/customfeeds.conf
...
# node_exporter
git clone https://$github/JohnsonRan/packages_utils_node_exporter package/new/node_exporter
